...
use crate::model::user::{LoginUser, NewUser, User};
...
#[post("/login")]
pub async fn login(db_pool: web::Data<Pool<Postgres>>, login_details: web::Json<LoginUser>) -> Result<impl Responder, actix_web::error::Error> {
	let new_token = LoginUser::login(&*login_details, &db_pool).await?;


	Ok(web::Json(new_token))
}